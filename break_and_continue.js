...
while (true) {
    let userNum = prompt("Enter odd number between 1 and 50");
    userNum = parseFloat(userNum); // parseFloat makes sure 'userNum' doesn't come back as string
    if (userNum % 2 === 1 && userNum <= 50 && userNum >= 1) {
        for (let i = 1; i <= 50; i += 2) {
            if (i === userNum) {
                console.log("yikes!" + userNum + " is not a number")
                continue;
            }
